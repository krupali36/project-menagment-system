...
const Sidebar = () => {
  ...
  const projectData = () => {
    axios.get('http://localhost:9001/api/projects/')
      .then((res) => {
  ...
  return (
    <div className='py-5'>
      ...
      <ul className='border-r border-gray-300 pr-2 mb-4'>
        <Link to="/reports" onClick={(e) => handleLocation(e)}>
          <li className={`px-5 py-1.5 mb-1 text-gray-600 font text-sm select-none hover:text-indigo-600 rounded transition-colors hover:bg-indigo-200/80 flex items-center gap-2 ${paramsWindow === 'reports' && 'text-indigo-600 bg-indigo-200/80'}`}>
