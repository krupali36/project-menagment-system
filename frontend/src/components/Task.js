...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`http://localhost:9001/api/project/${projectId}`)
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`http://localhost:9001/api/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`http://localhost:9001/api/project/${projectId}/task/${taskId}`)
            .then((res) => {
        ...
        <div className='px-12 py-6 w-full'>
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-3">
                    <Link to={`/dashboard/${projectId}`} className="py-2 px-4 bg-indigo-700 text-white rounded-md hover:bg-indigo-800 transition-colors text-sm">
                        View Dashboard
                    </Link>
